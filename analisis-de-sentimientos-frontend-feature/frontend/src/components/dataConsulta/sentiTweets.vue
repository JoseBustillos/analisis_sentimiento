    ...
    <b-row>
      <b-col md="6" offset-md="3" class="text-right">
        ...
        <h3 class="text-white"><p>Sentimiento: {{this.$route.params.senti}}</p>
          <p>Tweets: {{this.$route.params.tweet}}</p></h3>
      </b-col>
      <!--{{this.$route.params}}-->
    </b-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      async getTweets() {
        axios.get('http://127.0.0.1:8000/api/v1.0/tweets/' + this.$route.params.senti +
          '/' + this.$route.params.tweet.substring(1) + '.json')
          .then(response => {
